...
namespace Fungus
{
    ...
    [AddComponentMenu("")]
    [ExecuteInEditMode]
    public class FungusMove : iTweenCommand
    {
        ...
        protected override void OnEnable()
        {
           ...
           // scene.GetComponent<Player>().CurrentSprites[1, Convert.ToInt32(scene.GetComponent<Player>().FindSprite(targetobject.name))] = Convert.ToString(x);
           // scene.GetComponent<Player>().CurrentSprites[2, Convert.ToInt32(scene.GetComponent<Player>().FindSprite(targetobject.name))] = Convert.ToString(y);
            base.OnEnable();
